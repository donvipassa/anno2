    ...
    if (markupModified) {
      showModal(MODAL_TYPES.CONFIRM, 'Несохраненные изменения', 'У вас есть несохраненные изменения в разметке. Что вы хотите сделать?', [
        { 
          ...
          primary: true
  }, [showModal, closeModal, imageState.width, imageState.height, setMarkupFileName, setMarkupModifiedState, loadAnnotations, validateMarkupFileName, readFileAsText, validateYOLOData, convertYOLOToPixels]);
        { 
          text: 'Не сохранять', 
          action: () => {
            closeModal();
